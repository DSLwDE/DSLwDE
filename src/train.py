...
def train(
    ...
    for rep in range(num_repeat):
        ...
        logits = pd.DataFrame(0, index=yearmons, columns=data['log_tr'].columns).loc[backtest_start:backtest_end]
        ...
        for ind in (pbar := tqdm.tqdm(range(4, len(yearmons)))):
            ...
            with torch.no_grad():
                ...
                if yearmon < backtest_start:
                    ...
                    output = output[-1]
                    logits.loc[yearmon] = output.cpu().numpy()
    ...
    return all_backtest, all_weights, all_logits

def train_rolling_universe(
    yearmons,
    backtest_start,
    backtest_end,
    date2ind,
